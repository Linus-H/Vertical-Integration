...
class LogTimeDerivativeCP(TimeDerivative):
    ...
    def __call__(self, z, t):
        ...
        dT_dt = (R / C_p) * T * average.avg_backward_e1(Dlnp_Dt) \
        ...
        dw_dt = - R * T * derivative.diff_backward_n1_e1(lnp, self.delta_z) + self.g_z \
                ...
                + 1.5e-5 * R * T * np.exp(- average.avg_backward_e1(lnp)) * laplace.diff_n2_e2(w, self.delta_z)
...
class LogTimeDerivativeLorenz(TimeDerivative):
    ...
    def __call__(self, z, t):
        ...
        dT_dt = (R / C_p) * T * Dlnp_Dt - average.avg_forward_e1(w) * derivative.diff_n1_e2(T, self.delta_z)

        dlnp_dt = Dlnp_Dt - average.avg_forward_e1(w) * derivative.diff_n1_e2(lnp, self.delta_z)

        dw_dt = - R * average.avg_backward_e1(T) * derivative.diff_backward_n1_e1(lnp, self.delta_z) + self.g_z \
                - w * derivative.diff_n1_e2(w, self.delta_z)
